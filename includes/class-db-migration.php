...
class Peaches_Ecwid_DB_Migration {
	...
	private static function migrate_to_0_2_0() {
		...
		// Log migration start
		error_log('Peaches Ecwid: Starting migration to 0.2.0 - renaming product_ingredients to product_settings');
		...
		error_log("Peaches Ecwid: Migration to 0.2.0 completed safely. Updated {$updated_posts} posts from product_ingredients to product_settings");
	}
	...
	private static function migrate_to_0_2_1() {
		...
		error_log('Peaches Ecwid: Starting migration to 0.2.1 - removing product groups and setting up product lines');
		...
		error_log("Peaches Ecwid: Migration to 0.2.1 completed. Removed {$deleted_posts} product group posts and cleaned up meta data");
	}
	...
	private static function migrate_groups_to_lines() {
		...
		if (empty($groups)) {
			error_log('Peaches Ecwid: No product groups found to migrate');
			return;
		}

		error_log('Peaches Ecwid: Migrating ' . count($groups) . ' product groups to product lines');
		...
		foreach ($groups as $group) {
			...
			if (is_wp_error($term_data)) {
				error_log('Peaches Ecwid: Failed to create product line for group: ' . $group->post_title . ' - ' . $term_data->get_error_message());
				continue;
			}
			...
			$product_settings = get_posts(array(
			...
			foreach ($product_settings as $product_setting) {
				...
				error_log('Peaches Ecwid: Assigned product line ' . $group->post_title . ' to product setting ' . $product_setting->post_title);
			}

			error_log('Peaches Ecwid: Successfully migrated group "' . $group->post_title . '" to product line');
		}
	}
	...
	private static function migrate_to_0_2_2() {
		...
		// Log migration start
		error_log('Peaches Ecwid: Starting migration to 0.2.2 - renaming master_ingredient to product_ingredient');
		...
		error_log("Peaches Ecwid: Migration to 0.2.2 completed. Updated {$updated_posts} posts from master_ingredient to product_ingredient");
	}

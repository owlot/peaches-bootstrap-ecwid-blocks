...
class Peaches_Product_Media_Manager implements Peaches_Product_Media_Manager_Interface {
	...
	public function get_product_media_data($product_id, $tag_key, $size = 'large', $fallback = true) {
		...
		try {
			...
			if (!$raw_media_data) {
				...
				if ($fallback) {
					$fallback_url = $this->get_ecwid_image_by_position($product, 1);
					if ($fallback_url) {
		...
		switch ($media_type) {
			...
			case 'ecwid':
				if (isset($media_data['ecwid_position'])) {
					return $this->format_ecwid_media_data($media_data['ecwid_position'], $product);
				}
				break;
		}
	 ...
	 * @return array|null Formatted data or null if invalid.
	 */
	private function format_ecwid_media_data($ecwid_position, $product) {
		...
		$media_type = $this->determine_media_type_from_url($image_data['src']);
		...
		return array(
			...
			'alt'            => $image_data['alt'] ?: $product->name,
			'caption'        => '',
			'description'    => '',
			'mime_type'      => $this->guess_mime_type_from_url($image_data['src']),
			'type'           => $media_type,
			'source'         => 'ecwid',
			'ecwid_position' => $ecwid_position,
			'sizes'          => $sizes,
			'srcset'         => $image_data['srcset'],
			'sizes_attr'     => $image_data['sizes'], // HTML sizes attribute
			'responsive'     => true,
		);
	}
	...
	private function create_fallback_media_data($product, $fallback_url = null, $tag_key = '') {
		...
		if (!$fallback_url) {
			...
			if ($image_data) {
				...
				return array(
					'url'         => $fallback_url,
					'title'       => $product->name . (!empty($tag_key) ? ' - ' . $tag_key : '') . ' (fallback)',
					'alt'         => $image_data['alt'] ?: $product->name,
					'caption'     => '',
					'description' => '',

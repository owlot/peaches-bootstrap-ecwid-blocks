...
class Peaches_Ecwid_API implements Peaches_Ecwid_API_Interface {
	...
	public function search_products( $query, $options = array() ) {
		...
		if ( class_exists( 'Ecwid_Api_V3' ) ) {
			try {
				...
				$this->log_error( 'Error during product search: ' . $e->getMessage() );
			}
		}
	...
	public function get_categories( $options = array() ) {
		...
		if ( class_exists( 'Ecwid_Api_V3' ) ) {
			try {
				...
				$this->log_error( 'Error getting categories: ' . $e->getMessage() );
			}
		}
	...
	public function get_all_products( $options = array() ) {
		...
		if ( class_exists( 'Ecwid_Api_V3' ) ) {
			try {
				...
				$this->log_error( 'Error fetching all products: ' . $e->getMessage() );
	...
	public function invalidate_product_description_caches( $product_id ) {
		...
		$this->log_info(
			'Invalidated product description caches',
			array(
				'product_id'    => $product_id,
	...
	private function log_error( $message, $context = array() ) {
		if ( class_exists( 'Peaches_Ecwid_Utilities' ) ) {
			...
			error_log( '[Peaches Ecwid] [Ecwid API] ' . $message . ( empty( $context ) ? '' : ' - Context: ' . wp_json_encode( $context ) ) );
		}
	}
}

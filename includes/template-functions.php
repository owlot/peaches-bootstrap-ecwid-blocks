 ...
 * @param bool  $force_auto_sizes  Whether to force auto sizes attribute.
...
function peaches_generate_responsive_image_html( $media_data, $attributes = array(), $force_auto_sizes = false ) {
	...
	if ( class_exists( 'Peaches_Ecwid_Utilities' ) && Peaches_Ecwid_Utilities::is_debug_mode() ) {
		Peaches_Ecwid_Utilities::log_error( '[INFO] [Template Functions] peaches_generate_responsive_image_html - Entry with media_data', array( 'media_data' => $media_data ) );
		Peaches_Ecwid_Utilities::log_error( '[INFO] [Template Functions] peaches_generate_responsive_image_html - Entry with attributes', array( 'attributes' => $attributes ) );
	}

	if ( ! is_array( $media_data ) || empty( $media_data['url'] ) ) {
		if ( class_exists( 'Peaches_Ecwid_Utilities' ) && Peaches_Ecwid_Utilities::is_debug_mode() ) {
			Peaches_Ecwid_Utilities::log_error( '[INFO] [Template Functions] peaches_generate_responsive_image_html - FAILED: Invalid media_data or empty URL' );
		}
		return '';
	}
	...
	if ( class_exists( 'Peaches_Ecwid_Utilities' ) && Peaches_Ecwid_Utilities::is_debug_mode() ) {
		Peaches_Ecwid_Utilities::log_error( '[INFO] [Template Functions] peaches_generate_responsive_image_html - src and srcset', array( 'src' => $src, 'srcset' => $srcset ) );
	}
	...
	if ( class_exists( 'Peaches_Ecwid_Utilities' ) && Peaches_Ecwid_Utilities::is_debug_mode() ) {
		Peaches_Ecwid_Utilities::log_error( '[INFO] [Template Functions] peaches_generate_responsive_image_html - processed alt', array( 'alt' => $alt ) );
	}

	if ( empty( $src ) ) {
		if ( class_exists( 'Peaches_Ecwid_Utilities' ) && Peaches_Ecwid_Utilities::is_debug_mode() ) {
			Peaches_Ecwid_Utilities::log_error( '[INFO] [Template Functions] peaches_generate_responsive_image_html - FAILED: Empty src after processing' );
		}
		return '';
	}
	...
	if ( isset( $media_data['source'] ) && 'ecwid' === $media_data['source'] ) {
	...
	if ( class_exists( 'Peaches_Ecwid_Utilities' ) && Peaches_Ecwid_Utilities::is_debug_mode() ) {
		Peaches_Ecwid_Utilities::log_error( '[INFO] [Template Functions] peaches_generate_responsive_image_html - final_attributes', array( 'attributes' => $final_attributes ) );
	}
	...
	if ( class_exists( 'Peaches_Ecwid_Utilities' ) && Peaches_Ecwid_Utilities::is_debug_mode() ) {
		Peaches_Ecwid_Utilities::log_error( '[INFO] [Template Functions] peaches_generate_responsive_image_html - final_html', array( 'html' => $final_html ) );
	}
...
function peaches_the_product_media( $product_id, $tag_key, $size = 'large', $attributes = array(), $fallback = true ) {
	...
	if ( isset( $media_data['type'] ) && 'image' !== $media_data['type'] ) {
...
function peaches_get_available_media_tags( $category = '' ) {
	...
	if ( ! empty( $category ) ) {
		...
		foreach ( $all_tags as $tag_key => $tag_data ) {
			if ( isset( $tag_data['category'] ) && $category === $tag_data['category'] ) {
 ...
 * @since 0.4.4
 *
 * @param WP_Block|null $block     Optional block instance for context detection.
...
function peaches_generate_smart_sizes_attribute( $block = null, $context = 'product', $max_width = 400 ) {
	if ( ! class_exists( 'Peaches_Responsive_Sizes_Calculator' ) ) {
		...
		return 'product' === $context ? '(max-width: 575px) 50vw, (max-width: 767px) 33vw, 25vw' : 'auto';
	}
...
function peaches_the_responsive_product_media( $product_id, $tag_key, $context = 'gallery', $attributes = array(), $fallback = true ) {
	...
	switch ( $media_type ) {
		...
		case 'video':
			...
			echo esc_html__( 'Your browser does not support the video tag.', 'peaches' );
			echo '</video>';
			break;
		...
		case 'audio':
			...
			echo esc_html__( 'Your browser does not support the audio element.', 'peaches' );
			echo '</audio>';
			break;

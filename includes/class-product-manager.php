...
class Peaches_Product_Manager implements Peaches_Product_Manager_Interface {
	...
	public function enqueue_admin_scripts( $hook ) {
		...
		if ( $screen && $screen->is_block_editor ) {
			...
			wp_add_inline_script(
				...
				window.wpApiSettings.nonce = "' . esc_js( $rest_nonce ) . '";

				// Initialize wp.apiFetch with the settings
				if (window.wp && window.wp.apiFetch) {
					window.wp.apiFetch.use(window.wp.apiFetch.createNonceMiddleware("' . esc_js( $rest_nonce ) . '"));
			...
			wp_add_inline_script(
				...
				window.EcwidGutenbergParams.restNonce = "' . esc_js( $rest_nonce ) . '";
	...
	public function enqueue_frontend_scripts() {
		...
		wp_add_inline_script(
			...
			window.EcwidSettings.ajaxUrl = "' . esc_url_raw( admin_url( 'admin-ajax.php' ) ) . '";
	...
	public function generate_breadcrumbs( $product ) {
		...
		if ( isset( $product->categoryIds ) && ! empty( $product->categoryIds ) && is_array( $product->categoryIds ) ) {
			...
			if ( class_exists( 'Ecwid_Api_V3' ) ) {
				try {
					...
					$this->log_error( 'Error fetching category for breadcrumbs', array(
		...
		ob_start();
		?>
		<nav aria-label="<?php esc_attr_e( 'Breadcrumb', 'peaches' ); ?>">
			<ol class="breadcrumb">
				<!-- Home link -->
				<li class="breadcrumb-item">
					<a href="<?php echo esc_url( home_url() ); ?>">
						<?php esc_html_e( 'Home', 'peaches' ); ?>
					</a>
				</li>
	...
	private function log_info( $message, $context = array() ) {
		if ( class_exists( 'Peaches_Ecwid_Utilities' ) && Peaches_Ecwid_Utilities::is_debug_mode() ) {
			Peaches_Ecwid_Utilities::log_error( '[INFO] [Product Manager] ' . $message, $context );
		}
	}
	...
	private function log_error( $message, $context = array() ) {
		if ( class_exists( 'Peaches_Ecwid_Utilities' ) ) {
			Peaches_Ecwid_Utilities::log_error( '[Product Manager] ' . $message, $context );
		} else {
			// Fallback logging if utilities class is not available
			error_log( '[Peaches Ecwid] [Product Manager] ' . $message . ( empty( $context ) ? '' : ' - Context: ' . wp_json_encode( $context ) ) );
		}
	}
}

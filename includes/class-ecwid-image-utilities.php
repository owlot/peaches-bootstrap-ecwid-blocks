...
class Peaches_Ecwid_Image_Utilities {
	 ...
	 *
	 * @return array Image data with srcset and sizes, or null if not found
	 */
	...
	private static function process_ecwid_image_data($image_data, $context = 'gallery') {
		...
		// Generate sizes attribute based on context and available sizes
		$sizes_attribute = self::generate_smart_sizes_attribute($context, array_keys($available_sizes));
	...
	/**
	 * Generate smart sizes attribute based on context and available image sizes
	 *
	 * @since 1.1.0
	 *
	 * @param string $context         Context for the image
	 * @param array  $available_widths Array of available image widths
	 *
	 * @return string Sizes attribute value
	 */
	private static function generate_smart_sizes_attribute($context, $available_widths) {
		if (empty($available_widths)) {
			return self::generate_bootstrap_sizes_attribute($context);
		}

		// Get max available width for fallback
		$max_width = max($available_widths);

		switch ($context) {
			case 'thumbnail':
				return '(max-width: 575px) 150px, 150px';

			case 'gallery':
				return sprintf(
					'(max-width: 575px) 100vw, (max-width: 767px) 50vw, (max-width: 991px) 33vw, (max-width: 1199px) 25vw, %dpx',
					min(300, $max_width)
				);

			case 'hero':
				return sprintf(
					'(max-width: 575px) 100vw, (max-width: 767px) 100vw, (max-width: 991px) 75vw, %dpx',
					min(800, $max_width)
				);

			case 'product':
				return sprintf(
					'(max-width: 575px) 100vw, (max-width: 767px) 80vw, (max-width: 991px) 60vw, %dpx',
					min(600, $max_width)
				);

			default:
				return sprintf(
					'(max-width: 575px) 100vw, (max-width: 767px) 50vw, (max-width: 991px) 33vw, %dpx',
					min(400, $max_width)
				);
		}
	}
	 ...
	 * @since 1.1.0
	 *
	 * @param string $original_url Original Ecwid image URL
	 * @param array  $widths       Optional array of widths to generate (fallback)
	 * @param object $product_data Optional product data to use for real sizes
	...
	/**
	 * Get complete responsive image attributes using Ecwid product data
	 *
	 * @since 1.1.0
	 *
	 * @param object $product  Ecwid product object
	 ...
	 */
	public static function get_product_responsive_image_attributes($product, $position = 0, $args = array()) {
		$defaults = array(
			'context' => 'gallery',
			'alt'     => '',
			'class'   => '',
		...
		// Use the improved method to get image data
		...
		$attributes = array(
			'src'      => $image_data['src'],
			'alt'      => !empty($args['alt']) ? $args['alt'] : ($image_data['alt'] ?: $product->name),
			'loading'  => $args['loading'],
			'decoding' => 'async'
		);

		// Add responsive attributes if we have them
		...
		$attributes['data-image-position'] = $position;

		// Add store ID if detectable
		$store_id = self::get_ecwid_store_id($image_data['src']);
		if ($store_id) {
	...
	/**
	 * Generate Ecwid resized image URL for specific width
	 *
	 * @since 1.0.0
	 *
	 * @param string $original_url Original Ecwid image URL
	 ...
	 *
	 * @return string|false Resized URL or false if generation failed
	 */
	public static function generate_ecwid_resized_url($original_url, $width) {
		if (!self::is_ecwid_image($original_url) || !is_numeric($width)) {
			return false;
		}
		...
		// Ecwid URL patterns for resizing
		$params = array(
			'w' => $width,
			'h' => $width, // Square for most gallery images
			'fit' => 'crop',
			'quality' => 85
		);

		return self::add_ecwid_params($original_url, $params);
	}

	/**
	 * Add or update parameters in Ecwid image URL
	 *
	 * @since 1.0.0
	 *
	 ...
	 */
	public static function add_ecwid_params($original_url, $params) {
		if (empty($params)) {
			return $original_url;
		}
		...
		if (strpos($original_url, '?') !== false) {
			...
			parse_str(parse_url($original_url, PHP_URL_QUERY), $existing_params);
	 ...
	 * @return array Image attributes array
	 */
	public static function get_responsive_image_attributes($image_url, $args = array()) {
		$defaults = array(
			'widths'  => null,
			'context' => 'gallery',
			'alt'     => '',
		...
		// Add responsive attributes for Ecwid images
		if (self::is_ecwid_image($image_url)) {
			$srcset = self::generate_ecwid_srcset($image_url, $args['widths']);
			if ($srcset) {
			...
			// Add store ID if detectable
			$store_id = self::get_ecwid_store_id($image_url);
			if ($store_id) {
				$attributes['data-ecwid-store-id'] = $store_id;
			}
		}

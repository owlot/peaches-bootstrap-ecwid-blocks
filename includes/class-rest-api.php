...
class Peaches_REST_API {
	 ...
	 * @param Peaches_Ecwid_Responsive_Images   $responsive_images         Responsive images manager
	 */
	...
	public function register_routes() {
		...
		/**
		 * Responsive image data endpoint for Ecwid images
		 *
		 * @since 0.4.3
		 */
		register_rest_route(
			...
			array(
				...
				'callback'            => array( $this, 'get_responsive_image_data' ),
				'permission_callback' => array( $this, 'check_public_permissions' ),
				'args'                => array(
					'product_id' => array(
						'description' => __( 'Ecwid product ID (required).', 'peaches' ),
						'type'        => 'integer',
						'required'    => true,
						'minimum'     => 1,
					),
					'position' => array(
						'description' => __( 'Image position in product gallery (0 = main image).', 'peaches' ),
						'type'        => 'integer',
						'required'    => false,
						'default'     => 0,
						'minimum'     => 0,
					...
					'context' => array(
						'description' => __( 'Image context for sizing.', 'peaches' ),
						'type'        => 'string',
						'required'    => false,
						'default'     => 'gallery',
						'enum'        => array( 'thumbnail', 'gallery', 'hero', 'product', 'default' ),
						'sanitize_callback' => 'sanitize_text_field',
					),
				),
			)
		...
		/**
		 * Batch responsive image data endpoint for multiple Ecwid images
		 *
		 * @since 0.4.3
		 */
		register_rest_route(
			self::NAMESPACE,
			'/responsive-image-data/batch',
			array(
				'methods'             => WP_REST_Server::CREATABLE,
				'callback'            => array( $this, 'get_batch_responsive_image_data' ),
				'permission_callback' => array( $this, 'check_public_permissions' ),
				'args'                => array(
					'images' => array(
						'description' => __( 'Array of image data objects.', 'peaches' ),
						'type'        => 'array',
						'required'    => true,
						'items'       => array(
							'type'       => 'object',
	...
	/**
	 * Get responsive image data for Ecwid image
	 *
	 * @since 0.4.3
	 *
	 ...
	 */
	public function get_responsive_image_data($request) {
		try {
			...
			if (!$product_id || !is_numeric($product_id)) {
				return new WP_Error(
					'missing_product_id',
					__('Product ID is required for enhanced responsive images.', 'peaches'),
					array('status' => 400)
				);
			}
			...
			if (!$image_data) {
				return new WP_Error(
					'image_not_found',
					__('Image not found at specified position.', 'peaches'),
					array('status' => 404)
				);
			}
			...
			return new WP_REST_Response(
				array(
					...
					'data' => array(
						...
						'alt' => $image_data['alt'],
						'available_sizes' => $image_data['available_sizes'],
						'enhanced' => true,
					)
				),
				200
			...
			error_log('Peaches API: Error getting responsive image data: ' . $e->getMessage());
			return new WP_Error(
				'server_error',
	...
	/**
	 * Get batch responsive image data
	 *
	 * @since 0.4.3
	 *
	 * @param WP_REST_Request $request Request object
	 ...
	 * @return WP_REST_Response|WP_Error Response object or error
	 */
	public function get_batch_responsive_image_data($request) {
		try {
			...
			foreach ($images as $image_data) {
				...
				$context = $image_data['context'] ?? 'gallery';

				if (empty($image_url) || !Peaches_Ecwid_Image_Utilities::is_ecwid_image($image_url)) {
					$results[] = array(
						'original_url' => $image_url,
						'error' => 'Invalid Ecwid image URL'
					);
					continue;
				}
				...
				$results[] = array(
					'original_url' => $image_url,
					'srcset' => Peaches_Ecwid_Image_Utilities::generate_ecwid_srcset($image_url),
					'sizes' => Peaches_Ecwid_Image_Utilities::generate_bootstrap_sizes_attribute($context),
					'store_id' => Peaches_Ecwid_Image_Utilities::get_ecwid_store_id($image_url),
					'media_type' => Peaches_Ecwid_Image_Utilities::determine_media_type($image_url),
					'context' => $context
				);
			}
			...
			error_log('Peaches API: Error getting batch responsive image data: ' . $e->getMessage());
			return new WP_Error(
				'server_error',
				__('Internal server error.', 'peaches'),
				array('status' => 500)

 ...
 *
 * Manages custom sitemap generation for Ecwid products.
 *
 * @since 0.4.6
 */
class Peaches_Ecwid_Sitemap_Manager {
	...
	public function build_ecwid_url_mapping() {
		...
		try {
			...
			if ( class_exists( 'Ecwid_Api_V3' ) ) {
				...
				$products_response = $ecwid_api_v3->get_products(
					array(
						'enabled' => true,
						'limit'   => 100,
					)
	...
	private function is_sitemap_request() {
		...
		return (
			...
			isset( $_GET['sitemap'] ) ||
			false !== strpos( $_SERVER['REQUEST_URI'], 'sitemap' ) ||
			false !== strpos( $_SERVER['REQUEST_URI'], 'wp-sitemap' )
		);
	}
	...
	private function log_error( $message, $context = array() ) {
		if ( class_exists( 'Peaches_Ecwid_Utilities' ) ) {
			...
			error_log( '[Peaches Ecwid] [Sitemap Manager] ' . $message . ( empty( $context ) ? '' : ' - Context: ' . wp_json_encode( $context ) ) );
		}
	}
}
...
class Peaches_Ecwid_Sitemap_Provider extends WP_Sitemaps_Provider {
	...
	/**
	 * Initialize the sitemap provider.
	 *
	 * @since  0.4.6
	 * @return void
	 */
	 ...
	 * @param  string $subtype  Optional. Not applicable for Users but required for compatibility.
	...
	public function get_url_list( $page_num, $subtype = '' ) {
		...
		try {
			...
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				$this->log_error( 'Sitemap Provider Error', array( 'error' => $e->getMessage() ) );
			}
		}
	...
	private function log_error( $message, $context = array() ) {
		if ( class_exists( 'Peaches_Ecwid_Utilities' ) ) {
			...
			// Fallback logging if utilities class is not available.
			error_log( '[Peaches Ecwid] [Sitemap Provider] ' . $message . ( empty( $context ) ? '' : ' - Context: ' . wp_json_encode( $context ) ) );
		}
	}
}

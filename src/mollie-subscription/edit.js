	...
	return (
		<div className="subscription-plan-editor border p-3 mb-3 rounded">
			<div className="d-flex justify-content-between align-items-center mb-3">
				<h4 className="h6 mb-0">{__('Subscription Plan', 'peaches')} #{index + 1}</h4>
				<Button
					isDestructive
					isSmall
			...
			<div className="row">
				<div className="col-md-6">
					<TextControl
						...
						min="0"
						value={plan.amount || ''}
						onChange={(value) => updatePlan('amount', parseFloat(value) || 0)}
					/>
				</div>
				<div className="col-md-6">
					<SelectControl
						...
						options={Object.entries(window.PeachesMollieEditor?.currencies || {}).map(([code, name]) => ({
							label: `${code} - ${name}`,
							value: code
						}))}
						onChange={(value) => updatePlan('currency', value)}
					/>
				</div>
			</div>
			...
			<SelectControl
				...
				options={Object.entries(window.PeachesMollieEditor?.intervals || {}).map(([value, label]) => ({
					label,
					value
				}))}
	...
	return (
		<div {...blockProps}>
			<InspectorControls>
				...
				<PanelBody title={__('Subscription Plans', 'peaches')} initialOpen={true}>
				...
				<PanelBody title={__('Display Options', 'peaches')} initialOpen={false}>
					<ToggleControl
					...
					<SelectControl
						...
						options={[
							...
							{ label: __('Compact', 'peaches'), value: 'compact' }
						]}
						onChange={(value) => setAttributes({ layoutStyle: value })}
					/>
				</PanelBody>

				<PanelBody title={__('Button Settings', 'peaches')} initialOpen={false}>
					<TextControl
					...
					<SelectControl
						...
						value={buttonStyle}
						options={[
							{ label: __('Primary', 'peaches'), value: 'btn-primary' },
							{ label: __('Secondary', 'peaches'), value: 'btn-secondary' },
							{ label: __('Success', 'peaches'), value: 'btn-success' },
							{ label: __('Danger', 'peaches'), value: 'btn-danger' },
							{ label: __('Warning', 'peaches'), value: 'btn-warning' },
							{ label: __('Info', 'peaches'), value: 'btn-info' },
							{ label: __('Light', 'peaches'), value: 'btn-light' },
							{ label: __('Dark', 'peaches'), value: 'btn-dark' },
							{ label: __('Outline Primary', 'peaches'), value: 'btn-outline-primary' },
							{ label: __('Outline Secondary', 'peaches'), value: 'btn-outline-secondary' }
						]}
						onChange={(value) => setAttributes({ buttonStyle: value })}
					/>
				</PanelBody>

				<PanelBody title={__('Custom CSS', 'peaches')} initialOpen={false}>
					<TextControl
						label={__('Custom CSS', 'peaches')}
						value={customCSS}
						onChange={(value) => setAttributes({ customCSS: value })}
						help={__('Add custom CSS styles for this block', 'peaches')}
			...
			<div className="peaches-mollie-subscription-editor">
				{!mollieActive && (
					<Notice status="warning" isDismissible={false}>
						<p>
							{__('Mollie payment plugin is not active. Please install and activate a Mollie payment plugin to use subscription features.', 'peaches')}
						</p>
					</Notice>
				)}
				...
				{!selectedProductId && !hasProductDetailAncestor ? (
					<div className="text-center p-4 border border-dashed rounded">
						<h4>{__('Mollie Subscription Block', 'peaches')}</h4>
						<p className="text-muted mb-3">
							{__('Select a product to configure subscription options', 'peaches')}
						</p>
						<p className="text-muted small">
							{__('Use the Product Selection panel in the sidebar to choose an Ecwid product', 'peaches')}
						</p>
					</div>
					...
					<div className="text-center p-4 border rounded">
						...
						<p className="text-muted mb-3">
							{hasProductDetailAncestor
								...
								: `${__('Product ID:', 'peaches')} ${selectedProductId}`
							}
						</p>
						{productData && (
						...
						<p className="text-muted mb-3">
							{__('Add subscription plans to get started', 'peaches')}
						</p>
					...
					<div className="subscription-preview border p-4 rounded">
						<h4 className="mb-3">{__('Subscription Preview', 'peaches')}</h4>

						{productData && (
							<div className="mb-3 p-3 bg-light rounded">
								<h6 className="mb-1">{__('Selected Product:', 'peaches')}</h6>
								<p className="mb-0"><strong>{productData.name}</strong></p>
							</div>
						)}
						...
						{showDescription && (
							<div className="subscription-description mb-4 text-center">
								...
								<p className="text-muted small">
									{__('Get this product delivered automatically and save money with our subscription plans.', 'peaches')}
								</p>
							</div>
						)}

						<div className={`subscription-plans-preview ${layoutStyle}`}>
							{subscriptionPlans.map((plan, index) => (
								<div key={index} className={`plan-preview mb-3 p-3 border rounded ${layoutStyle === 'cards' ? 'bg-light' : ''}`}>
									<div className="d-flex justify-content-between align-items-center">
										<div className="plan-info">
											<h6 className="plan-title mb-1 fw-bold">{plan.name}</h6>
											{showPricing && (
												<p className="plan-price mb-1">
													<span className="h6 text-primary fw-bold me-1">
														{plan.currency} {parseFloat(plan.amount || 0).toFixed(2)}
													</span>
						...
						<div className="mt-3">
							<small className="text-muted">
								{__('Preview only - subscription functionality will work on the frontend', 'peaches')}
							</small>
						</div>
					</div>
				)}

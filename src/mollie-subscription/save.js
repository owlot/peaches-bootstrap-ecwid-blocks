...
export default function save({ attributes }) {
	...
	const renderCardLayout = () => (
		<div className="row g-4">
			{subscriptionPlans.map((plan, index) => {
				const colClass = subscriptionPlans.length === 1
						...
						<div className="card h-100 shadow-sm border-0 position-relative overflow-hidden">
							...
							<div className="card-body d-flex flex-column p-4">
											...
											<div className="display-6 fw-bold text-primary mb-1">
												...
												{plan.amount % 1 !== 0 && (
													<small className="fs-4">
														.{String((plan.amount % 1) * 100).padStart(2, '0')}
													</small>
												)}
											</div>
											<p className="text-muted mb-3">
												<small>
													{__('per', 'peaches')} {plan.interval}
												</small>
											</p>
										</>
									)}
								...
								<div className="d-grid mt-auto">
									...
									>
										<span
											data-wp-text={`state.isLoading && state.selectedPlan === ${index} ? '${__('Processing...', 'peaches')}' : '${buttonText}'`}
										>
											{buttonText}
										</span>
	...
	const renderListLayout = () => (
		<div className="list-group list-group-flush">
			{subscriptionPlans.map((plan, index) => (
				<div key={index} className="list-group-item list-group-item-action border-0 rounded-3 mb-3 shadow-sm">
	...
	const renderCompactLayout = () => (
		<div className="row g-3">
			{subscriptionPlans.map((plan, index) => {
				const colClass = subscriptionPlans.length > 2 ? 'col-12 col-sm-6 col-lg-4' : 'col-12 col-sm-6 col-lg-6';
					...
					<div key={index} className={colClass}>
						<div className="border rounded-3 p-3 h-100 bg-light bg-opacity-50">
							<div className="d-flex justify-content-between align-items-start mb-2">
								<div className="flex-grow-1">
									<h6 className="fw-semibold mb-1 text-dark">{plan.name}</h6>
									...
									{showPricing && (
										<p className="mb-2">
											...
											<small className="text-muted ms-1">/ {plan.interval}</small>
										</p>
									)}
	...
	return (
		<div {...blockProps}>
			...
			<div className="row justify-content-center">
				<div className={`col-12 ${subscriptionPlans.length <= 2 ? 'col-lg-8' : 'col-lg-10'}`}>
					{renderPlansLayout()}
				</div>
			</div>
			...
			>
				<div className="text-center">
					...
					<p className="text-muted">{__('Processing your subscription...', 'peaches')}</p>
				</div>
			</div>

	...
	actions: {
		...
		*selectPlan(event) {
			...
			try {
				...
				const productId = getProductIdWithFallback(context.selectedProductId);
				...
				const subscription = yield actions.createSubscriptionPromise(productId, plan, customerData);
				...
				context.error = error.message || 'An error occurred while processing your subscription';
			} finally {
		...
		*getCustomerData() {
			...
			try {
				const storedData = sessionStorage.getItem('peaches_customer_data');
				if (storedData) {
		...
		*showCustomerModal(plan, productId) {
		...
		createCustomerModal(plan, productId) {
			...
			form.addEventListener('submit', async (e) => {
				...
				try {
					...
					if (modal.querySelector('#save-customer-data').checked) {
						sessionStorage.setItem('peaches_customer_data', JSON.stringify(customerData));
					}

					// Create subscription using regular async/await
					const subscription = await actions.createSubscription(productId, plan, customerData);
		...
		createSubscriptionPromise(productId, plan, customerData) {
			...
			.then(result => {
				if (!result.success) {
					throw new Error(result.data || 'Subscription creation failed');
				}
				return result.data;
			});
		},
		...
		async createSubscription(productId, plan, customerData) {
			return this.createSubscriptionPromise(productId, plan, customerData);
		},
		...
		handleCheckoutReturn(result) {
			...
			switch (result) {
				case 'success':
					actions.showNotification('Subscription created successfully!', 'success');
					break;
				case 'cancelled':
					actions.showNotification('Subscription was cancelled.', 'warning');
					break;
				case 'failed':
					actions.showNotification('Subscription creation failed. Please try again.', 'error');
					break;
			}
		},

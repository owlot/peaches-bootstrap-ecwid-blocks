			...
			<div className="product-ingredients accordion">
				<template data-wp-each--ingredient="context.ingredients">
					<div className="accordion-item">
						<div className="accordion-header">
							<button
								class={`accordion-button ${!startOpened ? 'collapsed' : ''}`}
								type="button"
						...
						<div
							class={`accordion-collapse collapse ${startOpened ? 'show' : ''}`}
				...
				<div data-wp-bind--hidden="!context.isLoading" class="text-center my-3">
					<div class="spinner-border text-primary" role="status">
						<span class="visually-hidden">{__('Loading ingredients...', 'ecwid-shopping-cart')}</span>
					</div>
				</div>
			</div>
		</div>

...
const { state, actions } = store('peaches-ecwid-product-description', {
	state: {
		...
		get hasContent() {
			...
			return !!(context.descriptionContent && context.descriptionContent.trim());
		},
	...
	actions: {
		...
		*refreshDescription() {
			...
			if (!productData?.id) {
				console.warn('Product data not found for description refresh');
				return;
			}
			...
			try {
				...
				const urlWithLang = currentLanguage && currentLanguage !== 'en'
		...
		*updateDOM() {
			...
			const titleElement = element.ref.querySelector('.product-description-title');
			...
			// Update content
			const contentElement = element.ref.querySelector('.product-description-content');

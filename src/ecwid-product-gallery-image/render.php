...
if ( ! function_exists( 'peaches_gallery_fetch_wordpress_media' ) ) {
	function peaches_gallery_fetch_wordpress_media( $media_id ) {
		...
		if ( 0 === strpos( $mime_type, 'video/' ) ) {
			...
			$media_type = 'audio';
		} elseif ( 0 === strpos( $mime_type, 'application/pdf' ) || 0 === strpos( $mime_type, 'text/' ) ) {
			$media_type = 'document';
		}
	...
	if ( function_exists( 'peaches_get_product_media_data' ) ) {
		...
		if ( $ecwid_blocks ) {
			...
			if ( $ecwid_api ) {
				...
				echo '<!-- Debug: Product ID: ' . esc_html( $product_id ) . ' -->';
				echo '<!-- Debug: WordPress Post ID: ' . esc_html( $post_id ?: 'NOT FOUND' ) . ' -->';

				if ( $post_id ) {
					$product_media = get_post_meta( $post_id, '_product_media', true );
					echo '<!-- Debug: Product Media Count: ' . esc_html( is_array( $product_media ) ? count( $product_media ) : 'NOT ARRAY' ) . ' -->';
					if ( is_array( $product_media ) ) {
						$tag_found = false;
						foreach ( $product_media as $media_item ) {
							if ( isset( $media_item['tag_name'] ) && $selected_media_tag === $media_item['tag_name'] ) {
								$tag_found = true;
								echo '<!-- Debug: Found Tag Media Type: ' . esc_html( $media_item['media_type'] ?? 'NOT SET' ) . ' -->';
								echo '<!-- Debug: Found Tag Data Keys: ' . esc_html( implode( ', ', array_keys( $media_item ) ) ) . ' -->';

								// Add specific debugging for WordPress attachment.
								if ( isset( $media_item['attachment_id'] ) ) {
									$attachment_id = $media_item['attachment_id'];
									echo '<!-- Debug: Attachment ID: ' . esc_html( $attachment_id ) . ' -->';
									$attachment = get_post( $attachment_id );
									echo '<!-- Debug: Attachment Exists: ' . esc_html( $attachment ? 'YES' : 'NO' ) . ' -->';
									if ( $attachment ) {
										echo '<!-- Debug: Attachment Post Type: ' . esc_html( $attachment->post_type ) . ' -->';
										echo '<!-- Debug: Attachment Status: ' . esc_html( $attachment->post_status ) . ' -->';
										$mime_type = get_post_mime_type( $attachment_id );
										echo '<!-- Debug: Attachment MIME: ' . esc_html( $mime_type ?: 'NONE' ) . ' -->';
										$url = wp_get_attachment_url( $attachment_id );
										echo '<!-- Debug: Attachment URL: ' . esc_html( $url ?: 'NONE' ) . ' -->';
									}
								}
								break;
							}
						...
						if ( ! $tag_found ) {
							echo '<!-- Debug: Tag "' . esc_html( $selected_media_tag ) . '" NOT FOUND in product media -->';
						}
					}
				}
			}
...
if ( ! $media_data && 'none' !== $fallback_type ) {
	if ( 'tag' === $fallback_type && ! empty( $fallback_tag_key ) && ! empty( $product_id ) ) {
	...
	} elseif ( 'media' === $fallback_type && ! empty( $fallback_media_id ) ) {
...
<div <?php echo $wrapper_attributes; ?>>
	...
	<?php else : // Document. ?>
		<div class="document-container text-center p-4 border rounded">
			...
			<h5 class="document-title"><?php echo $media_alt ?: esc_html__( 'Document', 'peaches' ); ?></h5>

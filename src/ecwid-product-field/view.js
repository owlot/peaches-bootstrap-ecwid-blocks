				...
				switch (fieldType) {
					...
					case 'subtitle':
						...
						if (product.attributes) {
							const subtitleAttr = product.attributes.find(attr =>
								attr.name === 'Ondertitel' || attr.name === 'Subtitle'
							);
							value = subtitleAttr?.valueTranslated?.nl || subtitleAttr?.value || '';
						}
						break;
					...
					case 'price':
						...
						if (product.compareToPrice && product.compareToPrice > product.price) {
							value = `<span class="regular-price text-decoration-line-through me-2 text-muted">€ ${product.compareToPrice.toFixed(2).replace('.', ',')}</span>
									 <span class="sale-price text-danger">€ ${product.price.toFixed(2).replace('.', ',')}</span>`;
						} else {
					...
					case 'stock':
						...
						const stockText = product.inStock ?
							...
							wp.i18n.__('Out of Stock', 'ecwid-shopping-cart');
					...
					case 'custom':
						...
						if (customFieldKey && product.attributes) {
							const customField = product.attributes.find(attr =>
								attr.name === customFieldKey
							);
							value = customField?.valueTranslated?.nl || customField?.value || '';
						}
						break;

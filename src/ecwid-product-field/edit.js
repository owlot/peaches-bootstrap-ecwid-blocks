...
const FIELD_TYPES = [
	...
	{ label: __('Product Subtitle', 'ecwid-shopping-cart'), value: 'subtitle' },
	{ label: __('Product Price', 'ecwid-shopping-cart'), value: 'price' },
	{ label: __('Stock Status', 'ecwid-shopping-cart'), value: 'stock' },
	{ label: __('Product Description', 'ecwid-shopping-cart'), value: 'description' },
...
function ProductFieldEdit(props) {
	...
	const getPreviewText = () => {
		switch (fieldType) {
			...
			case 'description':
				return __('This is a sample product description...', 'ecwid-shopping-cart');
			case 'custom':
				return customFieldKey ?
					__('Custom Field:', 'ecwid-shopping-cart') + ' ' + customFieldKey :
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={__('Product Field Settings', 'ecwid-shopping-cart')}>
					<Notice status="info" isDismissible={false}>
						{__('This block displays a product field dynamically based on the product detail block.', 'ecwid-shopping-cart')}
					</Notice>
					...
					{fieldType === 'custom' && (
						<TextControl
							label={__('Custom Field Key', 'ecwid-shopping-cart')}
							value={customFieldKey}
							onChange={(value) => setAttributes({ customFieldKey: value })}
							help={__('Enter the key name of the Ecwid custom field', 'ecwid-shopping-cart')}
						/>
					)}

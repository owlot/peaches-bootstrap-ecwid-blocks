 ...
 */
function createResponsiveImage(imageData, productId, position, context = 'gallery') {
	const img = document.createElement('img');

	// Basic attributes
	img.src = imageData.src || imageData.url || imageData.image400pxUrl;
	img.alt = imageData.alt || productId ? `Product ${productId} Image` : 'Product Image';
 ...
 */
async function fetchEnhancedImageData(productId, position, context = 'gallery') {
	try {
		const response = await fetch('/wp-json/peaches/v1/responsive-image-data', {
			method: 'POST',
			headers: {

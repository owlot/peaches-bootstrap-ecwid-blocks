...
( function ( $ ) {
		...
		function attemptInitialization( attempt = 1 ) {
			...
			setTimeout( function () {
				if ( typeof wp === 'undefined' || ! wp.editor ) {
					console.error(
						'wp.editor not available on attempt',
						attempt
					);
				...
				if ( $collapsed.length > 0 ) {
					console.log(
						'Editor element is in collapsed area, expanding first:',
						editorId
					);
				...
				console.log(
					'Attempting wp.editor.initialize for:',
					editorId,
					'attempt:',
					attempt
				);
				...
				try {
					...
					console.log(
						'wp.editor.initialize call completed for:',
						editorId
					);
				} catch ( error ) {
					console.error(
						'Error initializing WordPress editor on attempt',
						attempt,
						':',
						error
					);
					if ( attempt < maxAttempts ) {
						...
						console.error(
							'Failed to initialize WordPress editor after',
							maxAttempts,
							'attempts for:',
							editorId
						);
			...
			.each( function () {
				...
				$select.find( 'option' ).each( function () {
					...
					if ( optionValue === '' ) {
						...
						selectedTypes.includes( optionValue ) &&
	...
	function updateDescriptionIndices() {
		...
		$( '#descriptions-container .description-item' ).each(
			function ( newIndex ) {
				const $item = $( this );
				...
				if ( oldIndex !== newIndex ) {
					...
					$item
						...
						.attr(
							'name',
							'product_descriptions[' + newIndex + '][type]'
						);
					$item
						.find( 'input[name*="[title]"]' )
						.attr(
							'name',
							'product_descriptions[' + newIndex + '][title]'
						);
					$item
						.find( 'textarea[name*="[content]"]' )
						.attr(
							'name',
							'product_descriptions[' + newIndex + '][content]'
						);

					// Update textarea ID
					const $textarea = $item.find(
						'textarea[id^="description_content_"]'
					);
	...
	$( document ).ready( function () {
		...
		console.log(
			'Total description items found:',
			$( '.description-item' ).length
		);
		console.log(
			'Total accordion items found:',
			$( '.accordion-item' ).length
		);
		console.log(
			'Total items with data-index:',
			$( '[data-index]' ).length
		);
		...
		console.log(
			'Product Descriptions JavaScript initializing. Existing indices:',
			existingIndices,
			'Initial index set to:',
			descriptionIndex
		);
		...
		$( '.description-item textarea[id^="description_content_"]' ).each(
			function () {
				...
				if ( editorId ) {
					...
					const $textarea = $( this );
					const hasWpEditor =
						$textarea.closest( '.wp-editor-wrap' ).length > 0;
					const hasTinyMCE =
						typeof tinymce !== 'undefined' &&
						tinymce.get( editorId );
					...
					if ( ! hasWpEditor && ! hasTinyMCE ) {
						console.log(
							'Initializing TinyMCE for plain textarea:',
							editorId
						);
						initializeWordPressEditor( editorId, false );
					} else {
						console.log(
							'Skipping initialization for existing editor:',
							editorId
						);
					}
				}
		...
		$( '#add-description' ).on( 'click', function ( e ) {
			...
			const newDescription = template.replace(
				/\{\{INDEX\}\}/g,
				nextIndex
			);
			...
			const $newItem = $(
				'#descriptions-container .description-item'
			).last();
		...
		$( document ).on(
			...
			function ( e ) {
				...
				const editorId = $card
					.find( 'textarea[id^="description_content_"]' )
					.attr( 'id' );
				...
				$card.fadeOut( 300, function () {
					...
					if (
						$( '#descriptions-container .accordion-item' )
							.length === 0
					) {
		...
		$( document ).on(
			'change',
			'.description-item select[name*="[type]"], .accordion-item select[name*="[type]"]',
			function () {
		...
		$( 'form' ).on( 'submit', function ( e ) {
			...
			$( '.accordion-item' ).each( function () {
				...
				if ( ! type ) {
					errors.push(
						'All descriptions must have a type selected.'
					);
					$item
						.find( 'select[name*="[type]"]' )
						.addClass( 'is-invalid' );
				...
				if ( type && usedTypes.includes( type ) ) {
					errors.push(
						`Duplicate description type: ${ type }. Each type can only be used once.`
					);
					$item
						.find( 'select[name*="[type]"]' )
						.addClass( 'is-invalid' );

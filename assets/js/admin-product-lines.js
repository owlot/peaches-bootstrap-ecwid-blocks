	...
	$( document ).ready( function () {
		...
		const isProductLinePage = (
			( typeof pagenow !== 'undefined' &&
			  ( pagenow === 'edit-product_line' || pagenow === 'product_line' || pagenow === 'edit-tags' || pagenow === 'term' ) ) ||
		...
		function createMediaItemHtml( index, tag, attachmentId ) {
			...
			const removeButtonHtml = hasMedia
				? `<button type="button" class="button remove-media-button">${ ProductLinesParams.removeText || 'Remove' }</button>`
				: '';
				...
				<div class="line-media-item" data-index="${ index }" style="border: 1px solid #ddd; padding: 15px; margin-bottom: 10px; background: #f9f9f9;">
					<div style="display: flex; gap: 15px; align-items: flex-start;">
						...
						<div class="media-preview" style="flex: 0 0 150px; text-align: center;">
							${ hasMedia ? '<!-- Media preview will be populated -->' : '<div style="width: 150px; height: 100px; background: #f0f0f0; border: 2px dashed #ccc; display: flex; align-items: center; justify-content: center; font-size: 12px; color: #666;">No media selected</div>' }
						</div>
					</div>
				</div>
			`;
		...
		function selectMedia( $button ) {
			...
			const mediaFrame = wp.media( {
				title: ProductLinesParams.selectMediaTitle || 'Select Line Media',
				button: {
					text: ProductLinesParams.selectMediaButton || 'Use this media',
				},
				multiple: false,
			} );
			...
			mediaFrame.on( 'select', function () {
				...
				const previewHtml = attachment.sizes && attachment.sizes.thumbnail
				...
				$button.text( ProductLinesParams.changeMediaText || 'Change Media' );

				// Show remove button if not already present
				if ( ! $container.find( '.remove-media-button' ).length ) {
					const removeText = ProductLinesParams.removeText || 'Remove';
					$button.after(
		...
		function removeMedia( $button ) {
			...
			$container.find( '.media-preview' ).html(
			...
			// Update button text using localized strings
			$container.find( '.select-media-button' ).text( ProductLinesParams.selectMediaText || 'Select Media' );
				...
				$.ajax( {
					...
					success: function( response ) {
						if ( response.success ) {
							...
							console.warn( 'Failed to mark attachment as line media:', response.data );
						}
					},
					error: function( xhr, status, error ) {
						console.error( 'AJAX error marking attachment as line media:', error );
					}
				} );
			}
		}
		...
		function initializeFormValidation() {
			...
			$form.on( 'submit', function ( e ) {
				...
				// Clear any previous error styling
				$( '.media-tag-field, .line-media-item' ).css( 'border-color', '' );
				...
				$( '#add-line-media' ).prop( 'disabled', true ).text( 'Saving...' );
			} );
		}
		...
		function initializeSortable() {
			if ( $( '#line-media-container' ).length && typeof $.fn.sortable !== 'undefined' ) {
		...
		function updateMediaIndices() {
			$( '.line-media-item' ).each( function( newIndex ) {
				...
				$item.find( 'input[name*="[tag]"]' ).attr( 'name', `line_media[${ newIndex }][tag]` );
				$item.find( 'input[name*="[attachment_id]"]' ).attr( 'name', `line_media[${ newIndex }][attachment_id]` );
			} );
		...
		function initializeExistingMediaPreviews() {
			$( '.line-media-item' ).each( function() {
				...
				if ( attachmentId ) {
					...
					$selectButton.text( ProductLinesParams.changeMediaText || 'Change Media' );
